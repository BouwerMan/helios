...
static bool
read_dma(sATADevice* device, uint16_t command, void* buffer, uint32_t lba, size_t sec_size, size_t sec_count);
...
bool ata_read_write(sATADevice* device, uint16_t op, void* buffer, uint32_t lba, size_t sec_size, size_t sec_count)
{
	...
	case COMMAND_READ_SEC:
		...
		break;
	case COMMAND_READ_DMA: return read_dma(device, command, buffer, lba, sec_size, sec_count);
...
static uint16_t get_command(sATADevice* device, uint16_t op)
{
	...
	switch (op) {
	case OP_READ:  return use_dma ? COMMAND_READ_DMA : COMMAND_READ_SEC;
	case OP_WRITE: return COMMAND_WRITE_SEC;
